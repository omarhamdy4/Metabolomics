...
##keep metabolites with missingness rate less than 50% (I hashed it because all were good no bad indices found + an error:))
#good.inx=apply(is.na(All_metabolites_mat), 1, sum)/nrow(All_metabolites_mat) <0.5
#sum(good.inx)
#All_metabolites_mat=All_metabolites_mat[,good.inx]
#dim(All_metabolites_mat)
...
#These are the top selected panel of metabolites
...
#plot(as.phylo(fit), cex = 0.7, label.offset =0.5)
library("ggdendro")
...
df_pca2 <- prcomp(t(deg_100_exp))
df_out2 <- as.data.frame(df_pca2$x)
ggplot(df_out2,aes(x=PC1,y=PC2,color=Metadata$`Factor Value[Disease]` ))+
geom_point()+ggtitle("")+labs(color='')+
geom_point(size=8,alpha=0.5)+ #Size and alpha just for fun
stat_ellipse(geom="polygon", level=0.95, alpha=0.2)+ #adding a stat
#ggplot2::geom_polygon(data=df_out,aes(x=PC1,y=PC2,fill = Metadata$`Factor Value[Disease]`), alpha = 0.3)+
